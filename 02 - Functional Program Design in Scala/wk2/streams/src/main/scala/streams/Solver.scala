...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors map {case (blk,move) => (blk, move :: history)} toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filter {case (blk, _) => !(explored contains blk)}
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      initial ++ from(states, states.foldLeft(explored)(_ + _._1))
    }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] =
    pathsFromStart filter {case (blk,_) => done(blk)}
